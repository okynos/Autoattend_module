...
/*
 ...
 function autoattend_email_teachers_attend($sess)
 function autoattend_email_teachers_key($sess)
 function autoattend_email_user($sess, $user, $status)
	...
	if ($users) {
		...
		foreach($users as $user) {
			...
			if ($status!='X') {
				...
				if ($result and $sentemail) autoattend_email_user($sess, $user, $status);
			}
			//
			if ($err_mesg) {
...
//		$users = autoattend_get_users_bystatus($sess->id, 'Y');
		...
		if ($users) {
			...
			foreach($users as $user) {
				...
				if ($result and $sentemail) autoattend_email_user($sess, $user, 'X');
			}
			//
			unset($rec);
		}
...
function autoattend_email_html($info, $mesg, $ttle)
{
	...
	$posthtml .= '<strong>'.get_string($ttle, 'block_autoattend').'</strong><br />'."\n";
...
function autoattend_email_teachers_attend($sess)
{
	...
	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		...
		if (function_exists('autoattendmod_send_email_teachers')) {
			...
			$starttm  = strftime(get_string('strftimehourmin', 'block_autoattend'), $sess->starttime + $TIME_OFFSET);
			$endtm    = strftime(get_string('strftimehourmin', 'block_autoattend'), $sess->endtime + $TIME_OFFSET);
...
function autoattend_email_user($sess, $user, $status)
{
	...
	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		//
		if (function_exists('autoattendmod_send_email_user')) {
			//
			$courseid = $sess->courseid;
			$course   = $DB->get_record('course', array('id'=>$courseid));
			$daytime  = strftime(get_string('strftimedmyw', 'block_autoattend'), $sess->sessdate+$TIME_OFFSET);
...
function  autoattend_get_namepattern($courseid)
{
	...
	$pattern = 'fullname';
	if ($courseid==0) return $pattern;

	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_get_namepattern')) {
			$pattern = autoattendmod_get_namepattern($courseid);
		}
	}
	return $pattern;
}
...
function  autoattend_disp_feedback($courseid)
{
	...
	$disp = true;
	if ($courseid==0) return $disp;

	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_disp_feedback')) {
			$disp = autoattendmod_disp_feedback($courseid);
		}
	}
	return $disp;
}
...
function  autoattend_is_email_enable($courseid)
{
	...
	$enable = false;
	if ($courseid==0) return $enable;

	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_is_email_enable')) {
			$enable = autoattendmod_is_email_enable($courseid);
		}
	}
	return $enable;
}
...
function  autoattend_is_email_key($courseid)
{
	...
	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_is_email_key')) {
			$enable = autoattendmod_is_email_key($courseid);
		}
	}
	return $enable;
}
...
function  autoattend_is_email_user($courseid)
{
	...
	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_is_email_user')) {
			$enable = autoattendmod_is_email_user($courseid);
		}
	}
	return $enable;
}
...
function  autoattend_is_email_allreports($courseid)
{
	...
	$allrep = false;
	if ($courseid==0) return $allrep;

	if (file_exists($CFG->dirroot.'/mod/autoattendmod/locallib.php')) {
		require_once($CFG->dirroot.'/mod/autoattendmod/locallib.php');
		if (function_exists('autoattendmod_is_email_allreports')) {
			$allrep = autoattendmod_is_email_allreports($courseid);
		}
	}
	return $allrep;
}
