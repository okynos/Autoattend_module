...
if ($mode==='multi' or $mode==='one') {
	...
	$starttime  = $shour*ONE_HOUR_TIME + $smin*ONE_MIN_TIME - $TIME_OFFSET;
	...
	//Variables que definen si hay un cambio de hora.
	$hourchangeonetozero = false;
	$hourchangezerotoone = false;
		...
		while ($sdate <= $enddate) {
			if($sdate < $startweek + ONE_WEEK_TIME) {
				...
				if(in_array(date('D',$sdate), $sdays)) {
					...
					//Si la generación de sesiones cae en un cambio de hora.
					if(strcmp(date('I', $startdate), date('I', $endtm))!=0){
						if(strcmp(date('I', $endtm), '0')==0){
							$hourchangeonetozero = true;
						}else{
							$hourchangezerotoone = true;
						}
					}

					// Si la generación de sesiones está entre un cambio de hora y no es horario de verano
					// Se suma una hora para cuadrar la resta que se le aplica por el cambio de hora.
					if($hourchangeonetozero && strcmp(date('I', $strtm), '0') == 0){
						$strtm += ONE_HOUR_TIME;
						$endtm += ONE_HOUR_TIME;
						$hourchangeonetozero = false;
					}else if($hourchangezerotoone && strcmp(date('I', $strtm), '1') == 0){
						$strtm -= ONE_HOUR_TIME;
						$endtm -= ONE_HOUR_TIME;
						$hourchangezerotoone = false;
					}
